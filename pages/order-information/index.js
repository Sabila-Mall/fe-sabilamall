...
import {
  apiGetPaymentMekariPay,
  apiConfirmPaymentMekariPay,
  apiCreateVirtualAccount,
  apiUpdateVirtualAccount,
} from "../../api/Payment";
...
const OrderInformation = ({ order }) => {
  ...
  const voucherDiscount =
    orderData?.shipping_cost == 0 ? 0 : orderData?.shipping_promo_amount;
    ...
    for (let i = 0; i < temporaryProducts?.length; i++) {
      ...
      tempOrderProducts = [
        ...tempOrderProducts,
        {
          ...
          image: getImageLink(temporaryProducts[i].products_image),
          discount:
  ...
  const ButtonBuatVirtualAccount = () => {
    ...
    return (
      <Button colorScheme="red" mt={2} onClick={handleButton}>
        {isLoading ? <Spinner /> : "Buat Virtual Account"}
      </Button>
    );
  };
  ...
  const ButtonUpdateVirtualAccount = () => {
    ...
    return (
      <Button colorScheme="red" mt={2} onClick={handleButton}>
        {isLoading ? <Spinner /> : "Update Virtual Account"}
      </Button>
    );
  };
              ...
              <Stack>
                  ...
                  >
                    {(() => {
                      if (
                        ...
                        ![5, 6].includes(orderData?.orders_status_id)
                      ) {
                        return (
                          <>
                            <Button
                    ...
                    <Flex>
                      ...
                      >
                        <Text>
                          {resiData?.found == 0
                            ? "Kurir: -"
                            : "Kurir: " + resiData?.waybill_courrier}
                        </Text>
                        {resiData != null && (
                          <Flex alignItems="center">
                            <Text>{`No. Resi:  ${
                              resiData?.found == 0 ? "-" : resi
                            }`}</Text>
                            {resiData?.found != 0 && (
                              <Tooltip
                ...
                >
                  ...
                  >
                    ...
                    {orderData?.handling_fee_admin != 0 &&
                    orderData?.handling_fee_admin != null ? (
                      <Flex mt="8px">
                        <Text>Admin Fee</Text>
                        <Spacer />
                        <Text>
                          + {currencyFormat(orderData.handling_fee_admin)}
                        </Text>
                      </Flex>
                    ) : (
                      <></>
