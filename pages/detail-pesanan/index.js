...
const DetailPesanan = () => {
  ...
  const handleSelectedPengirman = (selected) => {
    ...
    if (selected === "") {
      ...
      const { shipping_promo, rate, etd, destination } = tempPengiriman;
      const estimasi = etd;
      let discount_amount;

      if (!!shipping_promo) {
        if (shipping_promo.promos_fulldisc == 1) {
          discount_amount = rate;
        } else {
          discount_amount = shipping_promo.promos_amount ?? 0;
        }
      }
      setPengiriman({
